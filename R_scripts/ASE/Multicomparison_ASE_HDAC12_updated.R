...
# conditions controls plotting order, xa_call_condition defines the condition
# used to call Xa/Xi alleles, and control_condition is the reference used for
# statistical comparisons. Each non-control condition is compared with the
# control; all pairwise comparisons are not performed.
...
##  Configuration
base_dir <- "/home/jvk3/Desktop/HDAC_counts/All three datsets bulk and ASE/HDAC1 and 2/ASE"

ase_dir <- file.path(base_dir, "8_ASE_counts")

meta_path <- file.path(base_dir, "meta_table_HDAC12_ase.csv")

out_dir <- file.path(base_dir, "ase_outputs_Xi_fraction_HDAC12")
...
data.table::setnames(meta, make.names(names(meta)))
...
head(meta[, .(sample, condition, rep, filename, filepath)])

all(file.exists(meta$filepath))

# Edit these two lines per dataset
# Edit these per dataset
conditions         <- c("Scramble", "siHDAC1", "siHDAC2")   # plotting order
xa_call_condition  <- "Scramble"   # baseline used to call which allele is Xa/Xi
control_condition  <- "Scramble"   # reference condition for all statistical comparisons
# These are the same condition here, but don't have to be - see below.
#
# Capacitation conditions
# conditions <- c("Naive", "Cap. Day 3", "Cap. Day 7", "Primed")
# xa_call_condition <- "Primed"  # Reliable condition for calling Xa/Xi alleles
# control_condition <- "Naive"   # Reference for all comparisons and plots
